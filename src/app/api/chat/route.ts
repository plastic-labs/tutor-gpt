...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!rateLimitResult.allowed) {
      return new NextResponse(
        JSON.stringify({
          ...
          details:
            'You can make up to 8 chat requests per minute. Please wait a moment before sending another message.',
        }),
        {
          status: 429,
          headers: {
    ...
    log('error', 'Starting Response Loop')
